...
// Resolve deps from backend's node_modules
const mysql  = require(require.resolve('mysql2/promise', { paths: [path.join(__dirname, '../backend')] }));
const bcrypt = require(require.resolve('bcryptjs',       { paths: [path.join(__dirname, '../backend')] }));
...
async function seed() {
  ...
  try {
    // ── Static SQL (roles, genres, plans, artist profiles, albums, songs, etc.) ──
    ...
    // ── Hash passwords properly with bcryptjs ──────────────────────────────────
